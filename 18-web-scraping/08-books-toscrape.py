...
with open("books.csv", "w", newline="") as file:
    ...
    writer.writerow(["title", "category", "price", "href", "stock"])
    ...
    for side_category in side_categories:
        ...
        for page in range(from_pages, to_pages + 1):
            ...
            for article in articles:
                ...
                book = requests.get(f"{href}")
                book_soup = BeautifulSoup(book.content, "html.parser")
                stock = book_soup.find("p", class_="instock").text.strip()
                print(f"{title}|{category}|{price}|{href}|{stock}")
